...
* [`src/fetchimagefsm/`](src/fetchimagefsm/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/fetchimagefsm/core.cljs): contains the SPA entry point, `init`
* [`test/fetchimagefsm/`](test/fetchimagefsm/): test files (ClojureScript,
...
5. Clone this repo and open a terminal in the `fetchimagefsm` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/fetchimagefsm/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns fetchimagefsm.example
  (:require [fetchimagefsm.config :as config])
